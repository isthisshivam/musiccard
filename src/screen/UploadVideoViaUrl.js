...
import { uploadSongViaUrl } from "../redux/services/uploadSongVaiFileUrlService";
...
const UploadVideoViaUrl = (props) => {
  ...
  const getUserProfileInformation = async () => {
    userInfo = await CommonUtility.getInstance().getUserPersonalInformation();
    setAllUserDetails(userInfo);
  };
  const setAllUserDetails = async (loggedInUserData) => {
    const { location } = loggedInUserData;
    if (location) {
  ...
  const validateFields = () => {
    ...
    } else if (CommonUtility.getInstance().isEmpty(videoURL)) {
      message = "Please enter video url";
    }
    // else if (!CommonUtility.getInstance().isValidURL(videoUrl)) {
      ...
      <View style={globalStyle.view_flex_one}>
        ...
        >
          ...
          <TextInput
            ...
            style={[globalStyle.textInput]}
          ></TextInput>
          {!CommonUtility.getInstance().isEmpty(userInfo) ? (
            <>
              <Text
                style={[
                  globalStyle.email_text,
              ...
              <TextInput
                ...
                placeholder={`India -` + stateValue + ` - ` + city}
                editable={false}
                keyboardType="default"
                style={[globalStyle.textInput]}
              ></TextInput>
...
export default UploadVideoViaUrl;
