...
const FollowingCard = (props) => {
  ...
  console.log("FollowingCard==", props.item.item);
  return (
    <Pressable
      onPress={() => onClick(props?.item?.item?.followedUser?._id)}
      style={[
    ...
    >
      <View style={[globalStyle.flexRow, globalStyle.alignItems_center]}>
        <Image
          ...
          source={
            item?.item?.followedUser?.profilePhotoUrl
              ? { uri: item?.item?.followedUser?.profilePhotoUrl }
        ...
        >
          {item?.item?.followedUser?.firstname}
        </Text>
      </View>
      <Image
